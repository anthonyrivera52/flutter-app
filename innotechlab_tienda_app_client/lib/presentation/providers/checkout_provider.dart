...
final checkoutProvider = StateNotifierProvider<CheckoutNotifier, CheckoutState>((ref) {
  ...
  return CheckoutNotifier(clearCartUseCase, supabaseClient, ref);
});
...
class CheckoutNotifier extends StateNotifier<CheckoutState> {
  ...
  CheckoutNotifier(this._clearCartUseCase, this._supabaseClient, this._ref) : super(CheckoutState());
  ...
  Future<bool> confirmOrder() async {
    ...
    try {
      ...
      final orderResponse = await _supabaseClient.from('orders').insert({
        'user_id': userId,
        'total_amount': state.cartItems.fold(0.0, (sum, item) => sum + item.totalPrice),
